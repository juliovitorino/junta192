...
import 'package:junta192/br/com/jcv/linker/classes/campanhacashback/CampanhaCashbackPage.dart';
...
class CampanhaCashback extends StatefulWidget {
  dynamic _cartaofull;

  CampanhaCashback(this._cartaofull);

  @override
  _CampanhaCashbackState createState() => _CampanhaCashbackState();
}

class _CampanhaCashbackState extends State<CampanhaCashback> {
  String _token;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: <Widget>[
          ...
          CommomActionButton(titulo: "Gerenciar Cashback", onpressed:() {
            Navigator.push(context,
                MaterialPageRoute(builder: (context) => new CampanhaCashbackPage(int.parse(widget._cartaofull['id']))));
/*
            CommonShowDialogYesNo retorno = new CommonShowDialogYesNo(
              context: context,
              icon: Icon(Icons.help_outline, size: 120.0, color: Colors.red,),
              msg: "Tem realmente certeza de LIBERAR o cashback da campanha?",
              textYes: "Sim",
              textNo: "Não",
            );
            ...
            retorno.showDialogYesNo().then((onValue){
              if(retorno.getChoice() == 'Y'){
                print("id campanha: " + widget._cartaofull['id']);
                Navigator.push(context,
                    MaterialPageRoute(builder: (context) => new CampanhaCashbackPage(int.parse(widget._cartaofull['id']))));
              }
            }
